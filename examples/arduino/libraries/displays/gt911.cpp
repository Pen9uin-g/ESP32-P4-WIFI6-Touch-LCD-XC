...
esp_lcd_touch_handle_t touch_gt911_init(DEV_I2C_Port port)
{
    ...
    // INT and RST are not controlled by this polling driver, so either legal
    // GT911 address may be active. Probe the primary address first.
    delay(200);  // Allow the controller to finish power-on before probing.
    uint8_t address = 0;
